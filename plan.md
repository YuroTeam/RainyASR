...
**注意**：Task 1-15 及配套测试/CI 已完成，Task 16（PyInstaller 打包）待实现。主链路已调整为 DashScope 实时 WebSocket ASR；Task 5 的 PCM/WAV 编码工具保留为实时音频发送、调试录音与文件转写 fallback 的基础能力。
...
### 4. 背压与并发控制
- **ASR 连接**：Worker 只在本地检测到有效音频活动后开启 1 条实时 WebSocket 会话；连续静音后主动关闭，避免程序空闲常驻时持续消耗流式 ASR API
...
### 5. API 成本控制与本地门控
- **已实现（Task 12）**：本地 RMS 静音门控。完全无声或低于阈值时只保留本地 pre-roll，不启动 ASR 会话；检测到连续有效音频后启动 ASR，并补发 pre-roll；连续静音后关闭 ASR 会话
- **默认策略**：`silence_rms_threshold=0.005`, `speech_start_frames=2`, `silence_stop_ms=3000`, `preroll_ms=500`
- **局限**：RMS 只能区分“有无明显声音”，不能区分人声、音乐、音效或环境噪声
- **后续改进**：在 RMS 之后增加本地 VAD（优先评估 Silero VAD/ONNX；轻量 fallback 可评估 webrtcvad），将“有声音才开 ASR”升级为“疑似有人声才开 ASR”
- **文本后过滤**：final transcript 进入翻译前可继续过滤过短文本、重复文本、无意义语气词、低置信度文本（若 provider 后续提供 confidence）
...
| `src/rainyasr/worker.py` | CREATE | ✅ | 后台录音+按需实时 ASR 会话+翻译调度工作协程，含静音门控/pre-roll/背压处理 |
...
| `tests/test_worker.py` | CREATE | ✅ | Worker partial/final、去重、history、静音门控、pre-roll、停止清理测试 |
...
| `scripts/` | CREATE | ✅ | 手动验证脚本目录（test_capture.py、test_asr.py、test_translate.py、test_worker.py） |
...
- **Action**: `SubtitleWorker`：
  - 持有 `RealtimeASRProvider`, `TranslationProvider`，可选连接 `SubtitleWindow`，可选持有 `AudioRingBuffer` 用于调试/fallback
  - 配置边界：Worker 不主动加载/保存 `AppConfig`；只接收 provider 实例与运行参数，配置来源由 App/后续设置面板负责
  - `start(capture_audio=True)`：
    1. 检测音频设备，启动 `sounddevice.InputStream`
    2. 在音频回调中将 float32 frame 转 mono + RMS 统计 + PCM16 bytes
    3. 使用 `loop.call_soon_threadsafe()` 投递到有上限的 `asyncio.Queue`
    4. 默认启用本地 RMS 静音门控：无声时不启动 ASR 会话；检测到连续有效音频后启动 ASR，并补发 pre-roll；连续静音后关闭 ASR 会话
    5. 启动 `audio_sender_task` 按需调用 `asr.start()` / `asr.send_audio()` / `asr.stop()`
    6. 启动 `asr_event_task` 消费 partial/final transcript（仅 ASR 会话活跃时存在）
    7. 启动 `translation_task` 只翻译 final transcript
  ...
  - 已知局限：RMS 只能过滤静音，不能判断音频是否为人声；后续应增加本地 VAD 和 final 文本后过滤，进一步避免音乐/音效触发 ASR/翻译
...
- **Validate**:
  ...
  # 真实验证需要 API key + loopback 音频：
  # DASHSCOPE_API_KEY=... DEEPSEEK_API_KEY=... uv run python scripts/test_worker.py --real
  ```

#### Task 12 Code Review 记录

**Round 1（2026-05-23）** — 发现 6 个问题：
1. `_send_audio_item` 中 `pcm_bytes` 与 `preroll` 语义重叠，当前帧被静默忽略
2. `preroll_ms` 可能小于 `speech_start_frames * frame_ms`，导致开门确认阶段丢帧
3. `_put_queue_sentinel` 使用 `while True` 忙等
4. `_await_task` / `_cancel_task` 静默吞掉所有 `Exception`
5. 缺少背压触发 ASR 重连的测试
6. `scripts/test_worker.py` 未校验 `--fake`/`--real` 互斥性

**修复策略**：
- 不采用直接 `raise ValueError` 限制配置，而是将开门确认阶段的有效语音帧独立缓存到 `_pending_speech_audio_frames`，与静音上下文 `_preroll_audio_frames` 分离。这样既允许 `preroll_ms=0`，又保证任何 `speech_start_frames` 配置下都不丢帧。
- `_AudioQueueItem` 重构为单一 `audio_blocks: tuple[bytes, ...]`，彻底消除语义重叠。
- sentinel 投递改为 `asyncio.wait_for(queue.put, timeout=0.2)` + 超时后排空重试。
- 任务清理异常分类处理：`CancelledError` / `TimeoutError` / 其他异常分别记录，不再静默吞掉。
- 新增 `BlockingSendASRProvider`（双 `asyncio.Event` 模拟阻塞 send），补充背压重连测试。
- 新增 `preroll_ms=0, speech_start_frames=3` 测试，验证零 pre-roll 场景下起始帧不丢。
- `pyproject.toml` 增加 `[tool.logfire] ignore_no_config = true`，消除测试输出噪音。

**Round 2（2026-05-23）** — 确认所有修复通过，指出 1 个遗留边缘情况：
- `_put_queue_sentinel` 在极端情况下（队列持续被填满）可能无法放入 sentinel，导致 `_audio_sender_task` 悬空。建议后续在 `stop()` 中 `_await_task` 超时后 fallback 到 `_cancel_task`。

**审查结论**：修改后的 Task 12 质量高，可以合并。
...
- **Action**: `SettingsDialog(QDialog)`：
  ...
  - 实时音频参数（采样率、frame_ms、音频队列上限、silence_rms_threshold，带合理范围限制）
  - ASR 参数（模型、语言、音频格式；默认 `qwen3-asr-flash-realtime` + `pcm`）
  - 字幕样式（字体选择、大小滑动条、颜色选择器、透明度滑动条、窗口宽度、双语模式）
  - 快捷键绑定（QKeySequenceEdit）
  - 保存时调用 `AppConfig.save()`；API key 通过 `save_env_api_keys()` 写入 `.env`，两者均设置文件权限 `0o600`
- **Mirror**: PySide6 表单布局
- **Validate**:
  ```bash
...
**已实现（2026-05-23）**：
- 新增 `src/rainyasr/gui/settings_dialog.py`，四页签设置面板（API Keys / Audio / Recognition / Appearance）。
- 自定义分段按钮、滑块联动输入框、颜色选择器（自动计算对比文字色）、字体选择对话框。
- 表单校验分层：字段级即时校验 → `current_config()` 提取 → Pydantic `model_validate()` 最终验证。
- 配置与 API key 分离存储：`config.toml` 存偏好，`.env` 存密钥，均设 `0o600` 权限。
- 新增 `tests/test_settings_dialog.py`，覆盖初始化、值收集、边界校验、保存/拒绝流程、颜色/字体对话框。

#### Task 13 Code Review 记录

**Round 1（2026-05-24）** — 发现 3 个问题：
1. `setAccessibleName(unit)` 语义错误，屏幕阅读器读出 "Hz"/"ms" 等单位而非字段用途
2. 滑块与输入框双向绑定延迟——仅在 `editingFinished` 时同步，用户输入过程中滑块不更新
3. `_combo_value` 复杂逻辑缺少注释

**Round 2（2026-05-24）** — 修复确认：
- `_number_field_row` / `_float_field_row` / `_linked_slider_field` 新增 `accessible_name` 参数，传入 "Sample rate value" 等有意义名称
- `_linked_slider_field` 新增 `textChanged.connect(sync_slider_from_edit)` 实现即时同步，并增加 `hasAcceptableInput()` 校验防止无效输入导致滑块异常
- `_combo_value` 添加 `# Editable combos can keep the last item data while showing custom typed text.` 注释
- 新增 slider 即时同步测试、invalid hotkey 测试、accessibleName 断言
...
**已实现（2026-05-23）**：
- 新增 `src/rainyasr/hotkey.py`，提供 `GlobalHotkeyManager`、热键格式转换、macOS 辅助功能权限提示和幂等 `start()/stop()` 生命周期。
- 将 Task 13 保存的 `ctrl+shift+r` 格式转换为 `pynput` 需要的 `<ctrl>+<shift>+r` 格式，支持常见修饰键别名、功能键和命名键。
- 热键回调通过 `QMetaObject.invokeMethod(..., Qt.QueuedConnection)` 切回 Qt 线程，再执行字幕窗口显示/隐藏。
- 新增 `tests/test_hotkey.py`，覆盖格式转换、非法热键、注册/注销、Qt queued callback、macOS 权限提示和 listener 错误包装。

#### Task 14 Code Review 记录

**Round 1（2026-05-24）** — 发现 2 个问题：
1. `pynput` 的延迟导入在 `GlobalHotkeyManager.start()` 中通过 `except Exception` 捕获，`ImportError` 被泛化为 "Failed to register"，用户无法区分热键无效与依赖缺失
2. 热键别名映射缺少 `insert`、`printscreen`、`pause`、`capslock` 等常用键

**Round 2（2026-05-24）** — 修复确认：
- `start()` 中单独 `except ImportError`，错误消息包含 "dependency is unavailable"
- `_NAMED_KEY_ALIASES` 新增 `insert`/`ins`、`printscreen`/`prtsc`、`pause`、`capslock`/`caps_lock` 映射
- README 补充热键支持说明
- 新增 `test_start_reports_missing_keyboard_backend` 测试
...
**已实现（2026-05-23）**：
- 新增 `src/rainyasr/main.py` 与 `src/rainyasr/__main__.py`，支持 `uv run rainyasr` 和 `uv run python -m rainyasr`。
- `RainyASRController` 负责 logfire 配置、音频 loopback 预检测、Provider/Worker 构建、全局热键启动、系统托盘菜单、设置窗口应用、worker/hotkey 重启和退出清理。
- 启动缺少 API Key、找不到 loopback 设备、worker 启动失败时会弹出明确错误提示；Linux/macOS/Windows 音频提示按平台给出。
- 设置保存后会立即应用字幕样式；热键、音频、ASR、翻译目标或 API Key 变化时会重启对应运行组件。
- 新增 `tests/test_main.py`，覆盖 logfire 配置、worker/provider 参数传递、音频检测失败、缺少 API Key、设置变更重启 worker 和热键。

#### Task 15 Code Review 记录

**Round 1（2026-05-24）** — 发现 4 个问题：
1. `_schedule_controller_task` 接收已实例化的 `Coroutine`，当事件循环不可用时调用 `coroutine.close()`，存在协程被意外关闭的风险
2. `_worker_restart_required` 静态方法未说明为何不比较 `subtitle` 配置段
3. `install_terminal_signal_handlers` 中 `QTimer` 轮询用途不直观
4. `toggle_playback` 中 `_schedule_controller_task(self._toggle_playback())` 会立即执行协程，如果事件循环未就绪则协程被 close

**Round 2（2026-05-24）** — 修复确认：
- `_schedule_controller_task` 改为接收 `coroutine_factory: Callable[[], Coroutine]`，按需创建协程，避免提前实例化导致的 close 问题
- `_worker_restart_required` 添加 docstring 说明 subtitle 样式直接应用窗口无需重建 worker
- `signal_poll_timer.timeout` 处添加注释说明 Python 信号处理器需要定期执行 bytecode
- 日志消息从 "Cannot toggle playback" 泛化为 "Cannot schedule controller task"
- 新增 tray activation 测试
...
- [x] Task 1-12 完成，配套测试/CI 就绪
- [x] Task 13-15 完成
- [ ] Task 16（PyInstaller 打包）待实现
