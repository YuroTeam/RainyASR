...
RainyASR 是一款跨平台桌面字幕翻译工具，通过录制系统音频流，利用 Groq Whisper 进行语音识别、DeepSeek 进行翻译，最终以无边框悬浮窗展示双语字幕。本计划将原始 PRD 中的 8 个 Phase 细化为可独立验证的 16 个 Task，每个 Task 包含明确的输入、输出、验证命令和回退策略。
...
| File | Action | Why |
|---|---|---|
| `pyproject.toml` | UPDATE | 补全缺失依赖：sounddevice, qasync, pydantic, pynput |
...
| `src/rainyasr/providers/asr.py` | CREATE | Groq Whisper OpenAI 兼容实现 |
...
- **Action**: 用 Pydantic 定义 `AppConfig`，包含：
  - API 配置：`groq_api_key`, `deepseek_api_key`, `groq_base_url`, `deepseek_base_url`, `asr_model`, `translate_model`
...
- **Validate**:
  ...
  c.save()
  c2 = AppConfig.load()
  assert c2.groq_api_key == c.groq_api_key
  import os, stat
...
- **Action**: `GroqASRProvider`：
  - 使用 `openai.AsyncOpenAI(base_url="https://api.groq.com/openai/v1", api_key=...)`
  - `transcribe()` 接收 WAV bytes，调用 `/audio/transcriptions`
  - model: `whisper-large-v3`
  - 异常处理：`openai.APIError`, `openai.APITimeoutError`，重试 1 次
- **Mirror**: `openai` SDK async pattern
- **Validate**:
  ```bash
  uv run python scripts/test_asr.py
  # 读取 audio_test.wav，调用 API，终端打印识别文字
  ```
...
- **Action**: `SettingsDialog(QDialog)`：
  - API Key 输入（Groq + DeepSeek，QLineEdit + password 掩码）
