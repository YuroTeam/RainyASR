...
**注意**：Task 1-4 及配套测试/CI 已完成，后续从 Task 5 开始。主链路已调整为 DashScope 实时 WebSocket ASR；Task 5 仍保留为音频编码工具与文件转写 fallback。
...
| `dashscope` | 计划使用 | **已安装；建议升级到 `>=1.25.6`** | Qwen ASR 实时 WebSocket SDK |
...
- **推送粒度**：从 `sounddevice` 回调接收 float32 frame，转换为 PCM16 bytes 后持续发送到 WebSocket
...
- 实时主链路优先使用 `asyncio.Queue` 或 `loop.call_soon_threadsafe()` 将音频 frame 从回调线程投递到 asyncio/qasync 事件循环
...
| `src/rainyasr/audio/wav.py` | CREATE | ⏳ | PCM16/WAV 编码工具；实时 ASR 使用 PCM，WAV 用于调试和 fallback |
...
| `src/rainyasr/providers/__init__.py` | CREATE | ✅ | Provider 导出（空） |
...
- **Action**: 实现音频编码工具：
  - `float32_to_pcm16(audio_array: np.ndarray) -> bytes`
    - 输入 float32 数组（范围 [-1, 1]）
    - clip 到 [-1, 1]，转换为 16-bit little-endian PCM bytes
...
- **Action**: 编写临时脚本 `scripts/test_capture.py`：
  - 检测设备 → 启动录制 10 秒
  - 同时验证两条输出：
    1. 实时路径：持续产生 PCM16 frame，打印 frame 计数和 RMS/峰值
    2. 调试路径：保存 `audio_test.wav`
  - 用播放器验证 WAV 内容是否为系统音频（而非麦克风）
- **Mirror**: 独立验证脚本
- **Validate**: 手动播放 `audio_test.wav` 确认内容是系统音频；终端持续打印非零音量 frame
...
- **Action**: `QwenRealtimeASRProvider`：
  ...
  - `events()`：异步产出 `TranscriptEvent(text, is_final)`
...
- **Action**: `SubtitleWorker`：
  ...
  - partial transcript：通过 Qt Signal 更新原文临时字幕，不进入翻译请求
  - final transcript：去重后进入翻译队列，携带最近 2 句 history 调用 `translate.translate()`
...
- [x] Task 1-4 完成，配套测试/CI 就绪
- [ ] Task 5-16 完成
- [x] `uv run ruff check src/` 零错误
- [x] macOS 音频设备检测验证通过（BlackHole 2ch）
